...
void syscallSemWait(struct StackFrame *sf) {
	...
	//putChar('0'+sem[(int32_t)sf->edx].value);
	int j=(int32_t)sf->edx;
	if (j<MAX_SEM_NUM)
	{
		...
		if (sem[j].value<0)
		{
			...
			pcb[current].blocked.next=sem[j].pcb.next;
			pcb[current].blocked.prev=&(sem[j].pcb);
			sem[j].pcb.next=&(pcb[current].blocked);
			(pcb[current].blocked.next)->prev = &(pcb[current].blocked);
...
void syscallSemPost(struct StackFrame *sf) {
	...
	if (j<MAX_SEM_NUM)
	{
		...
		if (sem[j].value<=0)
		{
			ProcessTable *pcbtmp=NULL;
			//pcbtmp= (ProcessTable*)((uint32_t)(sem[j].pcb.prev));
			pcbtmp=(ProcessTable*)((uint32_t)(sem[j].pcb.prev) -
					(uint32_t)&(((ProcessTable*)0)->blocked));
			pcbtmp->state=STATE_RUNNABLE;
			pcbtmp->sleepTime=0;
			
			sem[j].pcb.prev=(sem[j].pcb.prev)->prev;
			(sem[j].pcb.prev)->next=&(sem[j].pcb);
			//pcb[current].blocked
			//del wait ,change to runnable
